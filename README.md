...
- 2026-09-24: **Calendar `v38` + Editor Next Free Slot `v5`** — plus jamais d'articles publiés déplacés : le rééquilibrage du calendrier (après chaque drop) ne réécrit plus que `future`/`draft`/`pending`, les créneaux des publiés restent occupés tels quels, et les heures déjà passées du jour redeviennent utilisables. Côté éditeur : purge de la réservation obsolète quand un planifié passe en publié (cron), garde-fou si l'article est déjà publié (le fix manuel du 22/09 en ligne est conservé), et tout choix manuel de date (ex. 11h passée) lève la réservation au lieu d'être écrasé par le créneau auto. Le snippet « Default Next Date 10-14 Priority » est désactivé en prod. `Calendar v36/v37` et `Editor v3/v4` archivés, déployé sur mondary.design (#405, #406).
