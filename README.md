...
## 🧾 Changelog
- 2026-09-08: **Stats Page `2026.09.9`** — libellé version nommé (« Stats v2026.09.9 ») pour lever la confusion avec les versions du curseur. **Live Cursors `2026.09.14`** — version en tooltip du badge uniquement (badge/dropdown allégés). **Stats Page `2026.09.8`** — numéro de version déplacé en haut à droite du contenu, lisible (avant : fixed en bas à droite, invisible). **Live Cursors `2026.09.13`** — numéro de version visible sur la page : suffixe dans le badge « N en ligne · X vues », pied du dropdown et tooltip (parité avec `ss-version` de la page stats). **Live Cursors `2026.09.12`** — vrais noms quand possible : compte WP connecté, sinon nom du cookie de commentaire ; à défaut « Anonyme N » unique assigné côté serveur et mémorisé en localStorage (fini les faux prénoms aléatoires, plus de collisions).
