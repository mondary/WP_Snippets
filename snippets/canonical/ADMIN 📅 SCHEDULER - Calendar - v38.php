<?php
/*
 * Display name: ADMIN 📅 SCHEDULER - Calendar - v38
 * Scope: global
 */
...
function scheduled_posts_calendar_styles_alpha() {
    ...
    <style>
        ...
        /* V35: hero button + toolbar 2 lignes. */
        .calendar-header-row .calendar-search {
            flex: 1 1 0%;
            min-width: 0;
        }
        .calendar-actions {
        ...
        .calendar-stats > .calendar-stat + .calendar-stat {
            border-left: 1px solid #dcdcde;
        }

        .calendar-stats .dashicons {
            font-size: 16px;
            width: 16px;
            height: 16px;
            color: #787c82;
        ...
        @media (max-width: 480px) {
            ...
            .calendar-actions #reallocateDrafts {
                grid-column: 1 / -1;
            }
...
function generate_scheduled_posts_calendar_alpha() {
    ...
    <div class="wrap">
        <div class="calendar-hero">
            <h1>Calendrier <span style="font-size:0.55em;font-weight:600;color:#2271b1;vertical-align:middle;background:#e8f0fe;padding:2px 8px;border-radius:999px;margin-left:6px;">v38</span></h1>
            <button id="reallocateDrafts" type="button" title="Réallouer brouillons et planifiés dès aujourd'hui (slots partagés publish+future+draft)" class="button button-primary calendar-hero-btn">
                <span class="dashicons dashicons-randomize" aria-hidden="true"></span> Réallouer
            </button>
        </div>
        <div class="calendar-container" data-jetpack-boost="ignore">
          <div class="calendar-topbar">
            <div class="calendar-header">
                ...
                <div class="calendar-actions" role="group" aria-label="Actions du calendrier">
                    <button id="appendNextMonth" type="button" class="button" title="Ajouter le mois suivant à la vue">+1 mois</button>
                    <button id="showFullYear" type="button" class="button" title="Afficher l'année complète">Année complète</button>
                </div>
                ...
                <div class="calendar-stats" aria-label="Statistiques du calendrier">
                    <span class="calendar-stat" title="Total annuel : publiés, planifiés et brouillons (filtre catégorie inclus)">
                        <i class="dashicons dashicons-calendar-alt" aria-hidden="true"></i><span class="calendar-stat__label">Annuel</span><span id="totalYearPosts" class="count">0</span>
                    </span>
                    <span class="calendar-stat" title="Articles du mois sélectionné : publiés, planifiés et brouillons (filtre catégorie inclus)">
                        <i class="dashicons dashicons-calendar" aria-hidden="true"></i><span class="calendar-stat__label">Mois</span><span id="totalMonthPosts" class="count">0</span>
                    </span>
                    <span class="calendar-stat" title="Moyenne mensuelle : total annuel ÷ 12 (filtre catégorie inclus)">
                        <i class="dashicons dashicons-chart-area" aria-hidden="true"></i><span class="calendar-stat__label">Moy./mois</span><span id="avgPostsPerMonth" class="count">0</span>
                    </span>
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // V38 : les articles déjà PUBLIÉS ne sont jamais déplacés (leurs créneaux
        // restent occupés tels quels). Seuls future/draft/pending sont re-positionnés,
        // y compris sur des heures déjà passées de la journée.
        function buildDaySchedulePlan(posts, dayDate) {
            const publishedHours = new Set(
                posts
                    .filter(post => post.status === 'publish')
                    .map(post => new Date(post.date).getHours())
            );

            const movablePosts = [...posts]
                .filter(post => post.status !== 'publish')
                .sort((a, b) => {
            ...
            if (!movablePosts.length) {
                return [];
            }

            const freeHours = AUTO_SCHEDULE_ALLOWED_HOURS.filter(hour => !publishedHours.has(hour));
            const maxSlots = freeHours.length;

            return movablePosts.slice(0, maxSlots).map((post, index) => {
        ...
        function fetchYearStats(date) {
            ...
            if (selectedCategory) {
                statsParams.set('categories', selectedCategory);
            }

            return fetch(`<?php echo esc_url(rest_url('wp/v2/posts')); ?>?${statsParams}`, {
                headers: {
                    'X-WP-Nonce': '<?php echo wp_create_nonce('wp_rest'); ?>'
                }
            })
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP ${response.status} lors du chargement des statistiques annuelles`);
                }

                const yearlyTotal = parseInt(response.headers.get('X-WP-Total') || '0', 10) || 0;
                return response.json().then(() => ({
                    yearlyTotal,
                    // V33 : moyenne sur l'année complète, même périmètre que la grille.
                    avgPostsPerMonth: (yearlyTotal / 12).toFixed(2)
                }));
            });
        }
        ...
        function updateCalendar(date) {
            const target = normalizeMonth(date);
            const monthDelta = (target.getFullYear() - currentDate.getFullYear()) * 12
                + (target.getMonth() - currentDate.getMonth());

            if (visibleMonths.length <= 1) {
                visibleMonths = [target];
                currentViewMode = 'single';
            } else if (monthDelta !== 0) {
                // V32 : décale toute la fenêtre visible pour suivre la navigation
                visibleMonths = visibleMonths.map(monthDate =>
                    new Date(monthDate.getFullYear(), monthDate.getMonth() + monthDelta, 1)
                );
            }
            currentDate = target;
            monthSelect.value = target.getMonth();
            yearSelect.value = target.getFullYear();
            pendingCenterToday = false;
            refreshCurrentView();
        }
        ...
        document.getElementById('prevMonth').addEventListener('click', () => {
            updateCalendar(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1));
        });
        ...
        monthSelect.addEventListener('change', function() {
            updateCalendar(new Date(currentDate.getFullYear(), parseInt(this.value, 10), 1));
        });

        yearSelect.addEventListener('change', function() {
            updateCalendar(new Date(parseInt(this.value, 10), currentDate.getMonth(), 1));
        });
        ...
        function updatePostDate(postId, newDate, monthOffset = 0, sourceDate = '') {
            ...
            .then(() => {
                // Le post déplacé vient déjà de recevoir son slot final :
                // ne pas le réécrire lors du rééquilibrage du jour cible.
                const rebalances = [rebalanceDaySchedule(newDate, postId)];
