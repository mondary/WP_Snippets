<?php
/*
 * Display name: POST 📌 FLOATING - Google News Button - v3
 * Scope: global
 */
 ...
 * v3 : Le masquage au scroll du GTranslate est gere ici (classe .gt-hidden),
 *      car le snippet historique "GTranslate White" est archive et inactif.
 *      Ce snippet est donc autonome : positionnement + comportement scroll.
 *
...
function md_gnews_button_styles() {
    ...
    <style>
        ...
        /* Masque le switcher GTranslate au scroll (declenche par le JS plus bas). */
...
// Masque le switcher GTranslate des qu'on scrolle (ajoute/retire la classe .gt-hidden).
add_action( 'wp_footer', 'md_gnews_hide_gtranslate_on_scroll' );
function md_gnews_hide_gtranslate_on_scroll() {
    ?>
    <script>
        (function() {
