...
/*
 * Display name: ADMIN 📅 SCHEDULER - Editor Next Free Slot - v5
 * Scope: admin
 */
...
function clm_editor_get_next_free_slot_ajax() {
    ...
    // Réserver le créneau côté serveur : l'état Gutenberg peut être enregistré
    // avant que dispatch.editPost() soit effectivement persisté.
    if ( $post_id ) {
        update_post_meta( $post_id, '_clm_editor_reserved_slot', $slot->format( 'Y-m-d H:i:s' ) );
    }
...
/**
 * V5 : supprime la réservation quand l'utilisateur choisit lui-même une date
 * (notamment une heure déjà passée du jour) — son choix prime sur le créneau auto.
 */
function clm_editor_clear_reserved_slot_ajax() {
    check_ajax_referer( 'clm_editor_next_free_slot', 'nonce' );
    ...
    delete_post_meta( $post_id, '_clm_editor_reserved_slot' );
    wp_send_json_success();
}
add_action( 'wp_ajax_clm_editor_clear_reserved_slot', 'clm_editor_clear_reserved_slot_ajax' );

/**
 * V5 : quand un article planifié est publié automatiquement (cron), sa réservation
 * de créneau devient obsolète — sans ce nettoyage, la prochaine modification de
 * l'article (désormais publié) le renverrait dans le créneau réservé du futur.
 */
function clm_editor_clear_reserved_slot_on_publish( $post ) {
    if ( $post instanceof WP_Post ) {
        delete_post_meta( $post->ID, '_clm_editor_reserved_slot' );
    }
}
add_action( 'future_to_publish', 'clm_editor_clear_reserved_slot_on_publish' );

/**
 * Empêche une validation Gutenberg de publier provisoirement à la date du jour
 * alors qu'un créneau futur a déjà été réservé par l'éditeur.
 * V5 : ne s'applique jamais à un article déjà publié (meta résiduelle).
 */
function clm_editor_force_reserved_future_slot( $data, $postarr ) {
    $post_id = isset( $postarr['ID'] ) ? absint( $postarr['ID'] ) : 0;
    if ( ! $post_id ) {
        return $data;
    }

    $reserved = get_post_meta( $post_id, '_clm_editor_reserved_slot', true );
    ...
    $timestamp = wp_date( 'U', strtotime( $reserved ), wp_timezone() );
    if ( ! $timestamp || $timestamp <= current_time( 'timestamp' ) ) {
        delete_post_meta( $post_id, '_clm_editor_reserved_slot' );
        return $data;
    }

    // FIX 2026-09-22 : respecter un choix de statut explicite (brouillon, planifie, pending).
    // Le forçage ne vise que le cas d'origine du garde-fou : une validation Gutenberg
    // qui publierait (« publish ») à la date du jour alors qu'un créneau futur est
    // réservé. Avant ce correctif, TOUTE sauvegarde d'un post avec réservation était
    // repassée de force en « future » : impossible de remettre un post en brouillon,
    // ce qui empêchait aussi toute suppression côté éditeur.
    if ( ( $data['post_status'] ?? '' ) === 'publish' ) {
        $data['post_date']     = $reserved;
        $data['post_date_gmt'] = get_gmt_from_date( $reserved );
        $data['post_status']   = 'future';
    }

    return $data;
}
add_filter( 'wp_insert_post_data', 'clm_editor_force_reserved_future_slot', 10, 2 );
...
( function() {
    ...
    function getDateFromButton( button ) {
        const datasetDate = [ button, button.parentElement, button.closest( '[role="gridcell"]' ) ]
            .filter( Boolean )
            .map( function( element ) { return element.dataset.date || element.dataset.value || ''; } )
            .find( function( value ) { return /^\d{4}-\d{2}-\d{2}$/.test( value ); } );
        if ( datasetDate ) return datasetDate;
    ...
    function renderDateMarkers() {
        const dateCells = new Set(
            document.querySelectorAll(
                '.components-datetime__date, .components-datetime__calendar [role="gridcell"], .components-date-time-picker [role="gridcell"], .components-calendar__day, .components-calendar [role="gridcell"], .components-calendar button'
            )
        );
    ...
    let reservationGuard = null;

    // V5 : si l'utilisateur change ensuite la date à la main (par ex. une heure
    // déjà passée du jour), on lève la réservation serveur pour que son choix prime.
    function clearReservedSlot( postId ) {
        if ( ! postId ) return;
        const body = new URLSearchParams( {
            action: 'clm_editor_clear_reserved_slot',
            nonce: window.clmEditorNextFreeSlot.nonce,
    ...
    function startReservationGuard( appliedDate ) {
        if ( reservationGuard || ! appliedDate ) return;
        const postId = editor.getCurrentPostId();
        const target = String( appliedDate ).slice( 0, 16 );
        reservationGuard = window.wp.data.subscribe( function() {
            const current = editor.getEditedPostAttribute( 'date' );
            if ( current && String( current ).slice( 0, 16 ) !== target ) {
                reservationGuard();
                reservationGuard = null;
                clearReservedSlot( postId );
            }
        } );
    }
        ...
        window.fetch( window.clmEditorNextFreeSlot.ajaxUrl, {
            ...
            .then( function( response ) {
                if ( response.success && response.data && response.data.date ) {
                    startReservationGuard( response.data.date );
