<?php
/*
 * Display name: FRONTEND 🌸 FAB - Hub Flottant - v19
 * Scope: global
 */
 ...
 * Burger : icônes officielles iconmonstr layer-multiple-alt-filled et
 * cube-filled (licence iconmonstr libre) MORPHÉES via GSAP MorphSVGPlugin
 * (gratuit sur cdnjs, 3.13.0) a l'ouverture. Icône 30px dans le bouton de
 * 52px. IMPORTANT : mdhub_render tourne en wp_footer priorite 99 pour
 * s'executer APRES le chargement de GSAP (sinon morphBurger est muet).
 *
 ...
 * Historique : v19 = burger TOUJOURS animé : avec GSAP, morph du path
 * (0.7s, ease élastique) + rotation 180° du SVG ; sans GSAP (optimiseur),
 * spin CSS + échange du path à mi-course (mouvement visible au lieu du
 * saut sec de la v18) ; v18 = morph résistant aux optimiseurs
 * resynchronisation automatique de l'état courant dès que GSAP charge
 * (boucle d'attente 100ms, 20s max) ; v17 = icône burger 30px + fix
 * d'ordre wp_footer prio 99 ; v16 = burger en icônes officielles iconmonstr
 * layer-multiple-alt-filled <-> cube-filled avec MorphSVGPlugin (CDN) ;
 * v15 = icônes burger 46px + losanges redessinés (remplacés ici par les
 * vraies icônes) ; v14 = republication de la v13
...
function mdhub_print_styles() {
    ...
    <style>
        ...
        .mdhub-main-icons { position: relative; display: block; flex: 0 0 auto; width: 30px; height: 30px; transition: transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); }
        .mdhub-main-icons.is-swapping { transform: rotate(90deg) scale(0.72); }
...
// 3) Markup + scripts
// -----------------------------------------------------------------------------
add_action('wp_footer', 'mdhub_render', 99); /* apres le chargement des scripts GSAP (prio 20) */
function mdhub_render() {
    ...
    (function () {
        ...
        var iconWrap   = document.querySelector('.mdhub-main-icons');
        var BURGER_LAYER = <?php echo wp_json_encode($burger_layer); ?>;
        var BURGER_CUBE  = <?php echo wp_json_encode($burger_cube); ?>;

        var burgerState = null; /* dernier état demandé (true=ouvert) */

        function morphBurger(open) {
            burgerState = open;
            if (!burgerPath || !iconWrap) return;

            /* GSAP dispo : vrai morphing du path + rotation du badge */
            if (window.gsap && window.MorphSVGPlugin) {
                gsap.killTweensOf(burgerPath);
                gsap.killTweensOf(iconWrap);
                gsap.to(burgerPath, {
                    duration: open ? 0.7 : 0.55,
                    ease: open ? 'back.out(1.7)' : 'power2.inOut',
                    morphSVG: open ? BURGER_CUBE : BURGER_LAYER
                });
                gsap.fromTo(iconWrap, { rotation: open ? 0 : 90 }, {
                    rotation: open ? 180 : 0,
                    scale: open ? 1.12 : 1,
                    duration: 0.7,
                    ease: 'back.out(1.7)',
            ...
            /* Sans GSAP : spin CSS + échange du path a mi-course — l'échange
               n'est plus un saut sec, il est porté par la rotation */
            iconWrap.classList.add('is-swapping');
            clearTimeout(iconWrap.__swap);
            iconWrap.__swap = setTimeout(function () {
                burgerPath.setAttribute('d', open ? BURGER_CUBE : BURGER_LAYER);
                iconWrap.classList.remove('is-swapping');
            }, 280);
        }

        /* Les optimiseurs (WP Rocket, Autoptimize, Rocket Loader…) peuvent
           décharger GSAP après notre inline script : on resynchronise
           l'état courant dès qu'il est enfin disponible (20s max). */
        (function waitForGsap() {
            ...
            var iv = setInterval(function () {
                if (window.gsap && window.MorphSVGPlugin) {
                    ...
                    morphBurger(burgerState === null ? stackOpen() : burgerState);
                    clearInterval(iv);
