<?php
/*
 * Display name: FRONTEND 🌸 FAB - Hub Flottant - v1
 * Scope: global
 */
 ...
 * en menu "en petale" (deploiement en eventail, 2 rayons alternes) :
 *
...
function mdhub_print_styles() {
    ...
    <style>
        ...
        #mdhub-main {
            ...
            border: none;
            cursor: pointer;
            background: linear-gradient(145deg, #23242a, #101114);
            color: #fff;
            display: flex;
            align-items: center;
            justify-content: center;
            box-shadow: 0 6px 22px rgba(0, 0, 0, 0.30), inset 0 0 0 1px rgba(255, 255, 255, 0.07);
            transition: transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1), box-shadow 0.25s ease;
            z-index: 2;
        }
        #mdhub-main:hover, #mdhub-main:focus-visible {
            transform: scale(1.06);
            box-shadow: 0 8px 28px rgba(77, 124, 255, 0.40), inset 0 0 0 1px rgba(255, 255, 255, 0.10);
            outline: none;
        }
        #mdhub-main:focus-visible {
            box-shadow: 0 0 0 3px rgba(77, 124, 255, 0.45), 0 8px 28px rgba(77, 124, 255, 0.40);
        }
        /* Halo discret qui respire pour attirer l'oeil. */
        #mdhub-main::after {
            content: '';
            position: absolute;
            inset: -5px;
            border-radius: 50%;
            pointer-events: none;
            animation: mdhub-halo 3.4s ease-out infinite;
        }
        @keyframes mdhub-halo {
            0%   { box-shadow: 0 0 0 0 rgba(77, 124, 255, 0.30); }
            70%  { box-shadow: 0 0 0 15px rgba(77, 124, 255, 0); }
        ...
        #mdhub.is-open #mdhub-main svg { transform: rotate(135deg); }

        /* ---- Petales : eventail 2 rayons alternes ---- */
        .mdhub-item {
            position: absolute;
            right: 6px;
            bottom: 6px;
            ...
            transform: rotate(var(--a)) translate(14px) rotate(calc(-1 * var(--a))) scale(0.15);
            opacity: 0;
            visibility: hidden;
            pointer-events: none;
            transition:
        ...
        #mdhub.is-open .mdhub-item {
            transform: rotate(var(--a)) translate(var(--r)) rotate(calc(-1 * var(--a))) scale(1);
            opacity: 1;
            visibility: visible;
            pointer-events: auto;
            transition-delay: calc(var(--i) * 32ms);
        }
        #mdhub.is-open .mdhub-item:hover,
        #mdhub.is-open .mdhub-item:focus-visible {
            transform: rotate(var(--a)) translate(var(--r)) rotate(calc(-1 * var(--a))) scale(1.12);
            box-shadow: 0 10px 26px -6px var(--c, rgba(0, 0, 0, 0.3));
            outline: none;
        }
        .mdhub-item svg { width: 21px; height: 21px; }
        ...
        #mdhub.is-open .mdhub-label {
            opacity: 1;
            transform: translateY(-50%) translateX(0);
            transition-delay: calc(var(--i) * 32ms + 150ms);
        }
        ...
        @media screen and (max-width: 600px) {
            #mdhub { bottom: 16px; right: 16px; width: 52px; height: 52px; }
            .mdhub-item { width: 42px; height: 42px; right: 5px; bottom: 5px; }
            .mdhub-item svg { width: 19px; height: 19px; }
            .mdhub-label { display: none; }
            #mdhub-panel { bottom: 84px; right: 16px; padding: 13px 16px; }
            .gt_switcher_wrapper { bottom: 16px !important; left: 16px !important; }
        }

        @media (prefers-reduced-motion: reduce) {
            #mdhub-main::after { animation: none; }
            #mdhub-main, #mdhub-main svg,
            .mdhub-item, .mdhub-label, #mdhub-panel { transition: none; }
        }
...
function mdhub_render() {
    ...
    // ---- Definitions du menu (ordre = bas de l'eventail vers le haut) ----
    $items = array(
        array(
            ...
            'blank' => true,
            'c'     => '#4285F4',
            'svg'   => '<svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">'
        ...
        array(
            ...
            'blank' => true,
            'c'     => '#f28b30',
            'svg'   => '<svg viewBox="0 0 24 24" width="21" height="21" aria-hidden="true" fill="currentColor">'
                     . '<circle cx="6.2" cy="17.8" r="2.2"/>'
                     . '<path d="M5.2 10.4a8.4 8.4 0 0 1 8.4 8.4h-2.9a5.5 5.5 0 0 0-5.5-5.5v-2.9z"/>'
                     . '<path d="M5.2 4.4a14.4 14.4 0 0 1 14.4 14.4h-2.9A11.5 11.5 0 0 0 5.2 7.3V4.4z"/>'
                     . '</svg>',
        ),
        array(
            'label' => 'Newsletter',
            ...
            'blank' => false,
            'c'     => '#d94f8a',
            'svg'   => '<svg viewBox="0 0 24 24" width="21" height="21" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
        ...
        array(
            ...
            'action'=> 'player',
            'c'     => '#4d7cff',
            'svg'   => '<svg viewBox="0 0 24 24" width="21" height="21" aria-hidden="true"><path fill="currentColor" d="M8 5v14l11-7z"/></svg>',
        ),
        array(
        ...
        array(
            ...
            'blank' => false,
            'c'     => '#0f9d58',
            'svg'   => '<svg viewBox="0 0 24 24" width="21" height="21" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round">'
                     . '<path d="M5 20v-6M11 20V6M17 20v-10M2.5 20h19"/>'
                     . '</svg>',
        ),
        array(
            ...
            'svg'   => '<svg viewBox="0 0 24 24" width="21" height="21" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
                     ...
                     . '<path d="M17 9v6.5A4.5 4.5 0 0 1 12.5 20h-3A4.5 4.5 0 0 1 5 15.5V9"/>'
                     . '<path d="M5 9V7a4 4 0 0 1 4-4"/>'
                     . '<path fill="currentColor" stroke="none" d="M11.6 11.2c-.9-.9-2.4-.4-2.4.9 0 1.1 1.5 2 2.4 2.7.9-.7 2.4-1.6 2.4-2.7 0-1.3-1.5-1.8-2.4-.9z"/>'
                     . '</svg>',
        ),
        array(
            ...
            'action'=> 'top',
            'c'     => '#2d3748',
            'svg'   => '<svg viewBox="0 0 24 24" width="21" height="21" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">'
    ...
    <nav id="mdhub" aria-label="Menu du site">
        ...
        <div id="mdhub-petals">
            <?php foreach ($items as $i => $it) :
                $a   = 182 + $i * 12.5;              // eventail : 182deg -> 269.5deg
                $r   = ($i % 2 === 0) ? 105 : 160;   // 2 rayons alternes = effet petale
            ...
            ?>
            <<?php echo $tag; ?> class="mdhub-item" style="--a:<?php echo $a; ?>deg;--r:<?php echo $r; ?>px;--i:<?php echo $i; ?>;--c:<?php echo $it['c']; ?>"<?php echo $url . $blk; ?>>
                <span class="mdhub-label" aria-hidden="true"><?php echo $it['label']; ?></span>
                <?php echo $it['svg']; ?>
