<?php
/*
 * Display name: ADMIN - Schedule Calendar [DRAG+14h] + Reallocation v2 (MIN v19)
 * Scope: global
 */
...
function generate_scheduled_posts_calendar_alpha() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const AUTO_SCHEDULE_ALLOWED_HOURS = [10, 11, 12, 13, 14];
        ...
        function buildDaySchedulePlan(posts, dayDate) {
            ...
            const numPosts = Math.min(sortedPosts.length, maxSlots);
            const startIdx = maxSlots - numPosts;

            return sortedPosts.slice(0, maxSlots).map((post, index) => {
                const hour = allowedHours[startIdx + index];
        ...
        if (reallocateDraftsButton && reallocateDialog) {
            ...
            reallocateConfirm.addEventListener('click', function() {
                ...
                setReallocateStatus(`Traitement serveur en cours (${articlesPerDay} article(s)/jour)...`);
                ...
                .then(data => {
                    ...
                    setReallocateStatus(`Terminé. Déplacés: ${moved}/${total}. Échecs: ${failed}.`, failed > 0 || remainingTodayIds.length > 0);
                    const details = failed > 0 && failedIds ? `\nIDs en échec: ${failedIds}` : '';
                    const remainingDetails = remainingTodayIds ? `\nRestent aujourd'hui/avant: ${remainingTodayIds}` : '';
                    alert(`✅ Réallocation terminée !\nDéplacés: ${moved}/${total}\nÉchecs: ${failed}${details}${remainingDetails}`);
                    refreshCurrentView();
                })
                .catch(error => {
...
function clm_reallocate_overdue_posts_ajax() {
    ...
    $slot_hours = array( 10, 11, 12, 13, 14 );
    ...
    wp_send_json_success( array( 'moved' => $moved, 'failed' => $failed, 'failed_ids' => array_values( array_unique( $failed_ids ) ), 'total' => count( $draft_ids ), 'candidates' => count( $candidates ), 'today' => $today->format( 'Y-m-d H:i:s' ), 'next_slots' => array_slice( $next_slots, 0, 10 ), 'remaining_today_ids' => array_values( array_map( 'intval', $remaining_today_ids ) ), 'articles_per_day' => $articles_per_day ) );
}
