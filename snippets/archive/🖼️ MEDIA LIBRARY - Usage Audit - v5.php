...
if ( ! function_exists( 'clm_media_simple_where_used_boot' ) ) {
    ...
    function clm_media_simple_where_used_boot() {
        add_filter( 'manage_upload_columns', 'clm_media_simple_where_used_columns', PHP_INT_MAX );
        ...
        add_action( 'admin_head-upload.php', 'clm_media_simple_force_hide_mla_columns_css', PHP_INT_MAX );
        add_action( 'admin_footer-upload.php', 'clm_media_simple_force_hide_mla_columns_js', PHP_INT_MAX );
        add_filter( 'hidden_columns', 'clm_media_simple_hide_mla_in_screen_options', 10, 2 );
        add_filter( 'default_hidden_columns', 'clm_media_simple_hide_mla_in_default_hidden', 10, 2 );
    }
    ...
    function clm_media_simple_force_hide_mla_columns_css() {
        echo '<style>
            .wp-list-table .column-media_category,
            .wp-list-table .column-taxonomy-media_category,
            .wp-list-table .column-media_tag,
            .wp-list-table .column-taxonomy-media_tag,
            .wp-list-table .column-mla_meta { display:none !important; }
        </style>';
    }

    function clm_media_simple_force_hide_mla_columns_js() {
        ?>
        <script>
        (function() {
            var labelsToHide = ['Media Categories', 'Media Tags', 'MLA Meta'];
            var classToHide = ['media_category', 'taxonomy-media_category', 'media_tag', 'taxonomy-media_tag', 'mla_meta'];

            var table = document.querySelector('.wp-list-table');
            if (!table) return;

            var hideByClass = function() {
                classToHide.forEach(function(cn) {
                    table.querySelectorAll('.column-' + cn).forEach(function(el) {
                        el.style.display = 'none';
                    });
                });
            };

            var hideByHeaderLabel = function() {
                var ths = table.querySelectorAll('thead th, tfoot th');
    ...
    function clm_media_simple_hide_mla_in_screen_options( $hidden, $screen ) {
        if ( ! $screen || 'upload' !== $screen->id ) {
            return $hidden;
        }

        $forced_hidden = array( 'media_category', 'taxonomy-media_category', 'media_tag', 'taxonomy-media_tag', 'mla_meta' );
        $hidden = array_unique( array_merge( (array) $hidden, $forced_hidden ) );

        // Ensure custom column is not hidden.
        $hidden = array_values( array_diff( $hidden, array( 'clm_media_used_in' ) ) );
        return $hidden;
    }

    function clm_media_simple_hide_mla_in_default_hidden( $hidden, $screen ) {
        return clm_media_simple_hide_mla_in_screen_options( $hidden, $screen );
    }
    ...
    function clm_media_simple_content_posts( $attachment_id ) {
        ...
        foreach ( $rows as $row ) {
            ...
            if (
                clm_media_simple_contains( $content, $id_token ) ||
                clm_media_simple_contains( $content, $url ) ||
                ( $path && clm_media_simple_contains( $content, $path ) )
            ) {
                $ids[] = (int) $row->ID;
            }
        }
    ...
    function clm_media_simple_contains( $haystack, $needle ) {
        if ( '' === $needle ) {
            return true;
        }
