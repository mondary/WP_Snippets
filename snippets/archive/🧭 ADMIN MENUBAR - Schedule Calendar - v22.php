<?php
/*
 * Display name: ADMIN - Schedule Calendar + Reallocation v22
 * Scope: global
 */
...
function generate_scheduled_posts_calendar_alpha() {
    ?>
    <div class="wrap">
        <h1>Calendrier <span style="font-size:0.55em;font-weight:600;color:#2271b1;vertical-align:middle;background:#e8f0fe;padding:2px 8px;border-radius:999px;margin-left:6px;">v22</span></h1>
            ...
            <div class="reallocate-field">
                ...
                <div class="reallocate-options">
                    ...
                    <label class="reallocate-option">
                        ...
                        <span class="reallocate-option-hours">Compacte dès aujourd'hui (10h puis 11h...)</span>
                    </label>
                </div>
            </div>
            <div class="reallocate-field">
...
// V20: Normalisation des articles planifiés (future) sur les créneaux 10/14/11/12/13.
// - Regroupe par jour, trie par date, repositionne sur les slots dans l'ordre de priorité.
// - Débordement (> 5/jour) cascadé vers J+1 (et au-delà si nécessaire).
// - Les posts déjà sur le bon créneau sont laissés intacts.
function clm_normalize_future_posts_schedule( array $slot_hours ) {
    $tz = wp_timezone();
    ...
    while ( ( $overflow || $by_day ) && $safety-- > 0 ) {
        ...
        $to_assign    = array_slice( $all_today, 0, $max_per_day );
        $new_overflow = array_slice( $all_today, $max_per_day );
...
// V22: Compactage des articles planifiés (future) vers les premiers jours libres.
...
//   on saute ceux déjà passés (sinon on planifierait dans le passé).
// - Pour les jours suivants: ordre prioritaire [10,14,11,12,13].
// - Respecte $articles_per_day (plafond TOTAL). Remplit d'abord les trous puis avance.
function clm_compact_future_posts( array $slot_hours, $articles_per_day, DateTimeImmutable $start_day ) {
    ...
    $day_slots_for = function( DateTimeImmutable $day ) use ( $slot_hours, $max_slots, $start_key, $now ) {
        if ( $day->format( 'Y-m-d' ) === $start_key ) {
            // Aujourd'hui: chronologique, on saute les créneaux déjà passés.
        ...
        // Jour complet: ordre prioritaire.
        return array_slice( array_map( 'intval', $slot_hours ), 0, $max_slots );
    };
...
function clm_reallocate_overdue_posts_ajax() {
    ...
    foreach ( $draft_ids as $post_id ) {
        ...
        $free_slots = array_values( array_diff( $slot_hours, $taken_hours ) );
