...
// Réallocation serveur: tous les brouillons -> J+1 puis jours suivants, 10h et 14h.
function clm_reallocate_overdue_posts_ajax() {
    ...
    if ( ! $draft_ids ) wp_send_json_success( array( 'moved' => 0, 'failed' => 0, 'failed_ids' => array(), 'total' => 0, 'candidates' => 0, 'today' => $today->format( 'Y-m-d H:i:s' ), 'next_slots' => array(), 'remaining_today_ids' => array() ) );
    ...
    wp_send_json_success( array( 'moved' => $moved, 'failed' => $failed, 'failed_ids' => array_values( array_unique( $failed_ids ) ), 'total' => count( $draft_ids ), 'candidates' => count( $candidates ), 'today' => $today->format( 'Y-m-d H:i:s' ), 'next_slots' => array_slice( $next_slots, 0, 10 ), 'remaining_today_ids' => array_values( array_map( 'intval', $remaining_today_ids ) ) ) );
}
