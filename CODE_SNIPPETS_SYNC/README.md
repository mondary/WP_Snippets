...
3. Lance depuis la racine du repo:
   - `bash CODE_SNIPPETS_SYNC/scripts/push_wordpress.sh --dry-run --verbose` (test)
   - `bash CODE_SNIPPETS_SYNC/scripts/push_wordpress.sh --verbose` (push reel)
...
- Push direct (lit `CODE_SNIPPETS_SYNC/secrets/wp-sync.env`)
  - `bash CODE_SNIPPETS_SYNC/scripts/push_wordpress.sh --dry-run --verbose`
  - `bash CODE_SNIPPETS_SYNC/scripts/push_wordpress.sh --verbose`
- Generer l'import depuis les snippets canonique:
  - `php CODE_SNIPPETS_SYNC/scripts/build_code_snippets_import.php --snippets-dir=snippets/canonical --out=CODE_SNIPPETS_SYNC/imports/IMPORT-WORDPRESS.json`
- Comparer un export WordPress avec un dossier local:
  - `php CODE_SNIPPETS_SYNC/scripts/compare_code_snippets_export.php --export=CODE_SNIPPETS_SYNC/exports/mon-export.json --snippets-dir=snippets/canonical --report=CODE_SNIPPETS_SYNC/reports/compare-report.json`
