...
## 🧾 Changelog
- 2026-09-08: **Stats `2026.09.10` + Cursors `2026.09.15`** — both versions ("Stats v… · Cursor v…") now shown only on the stats page (cursor version read dynamically from the active snippet); nothing on the cursor badge anymore. **Cursors `2026.09.12`** — real names when available: logged-in WP account, else the comment-author cookie name; otherwise a unique server-assigned "Anonyme N" remembered in localStorage (random fake first names removed, no more name collisions).
