...
## 🧾 Changelog
- 2026-09-08: **Stats Page `2026.09.9`** — named version label ("Stats v2026.09.9") to remove confusion with the live-cursor versions. **Live Cursors `2026.09.14`** — version kept as badge tooltip only (badge/dropdown decluttered). **Stats Page `2026.09.8`** — version number moved to the top-right of the content, readable (before: fixed bottom-right, invisible). **Live Cursors `2026.09.13`** — version number now visible on the page: suffix in the "N online · X views" badge, dropdown footer line and tooltip (parity with the stats page `ss-version`). **Live Cursors `2026.09.12`** — real names when available: logged-in WP account, else the comment-author cookie name; otherwise a unique server-assigned "Anonyme N" remembered in localStorage (random fake first names removed, no more name collisions).
