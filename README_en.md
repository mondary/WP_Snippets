...
## 🧾 Changelog
- 2026-09-08: **Stats `2026.09.11` + Cursors `2026.09.15`** — versions stacked on two right-aligned lines, shown only on the stats page (cursor version read dynamically from the active snippet); nothing on the cursor badge. **Cursors `2026.09.12`** — real names when available: logged-in WP account, else the comment-author cookie name; otherwise a unique server-assigned "Anonyme N" remembered in localStorage (random fake first names removed, no more name collisions).
