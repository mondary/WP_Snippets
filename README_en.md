...
## 🧾 Changelog
- 2026-09-24: **Calendar `v38` + Editor Next Free Slot `v5`** — published posts are never moved again: the calendar rebalance (after every drag & drop) now only rewrites `future`/`draft`/`pending`, published slots stay occupied as-is, and already-pasted hours of the day become usable again. Editor side: stale reservation is purged when a scheduled post auto-publishes (cron), a guard skips already-published posts (the manual online fix of 09/22 is preserved), and any manual date choice (e.g. a past 11:00) clears the reservation instead of being clobbered by the auto slot. The "Default Next Date 10-14 Priority" snippet is deactivated in prod. `Calendar v36/v37` and `Editor v3/v4` archived, deployed on mondary.design (#405, #406).
- 2026-09-08: **Action bar `v4`** — the petal FAB is replaced by a compact sticky bar, readable on desktop and horizontally scrollable on mobile; **Cursors `2026.09.16`** — top-right badge removed, live counter delegated to the hub. **Stats `2026.09.11` + Cursors `2026.09.15`** — versions stacked on two right-aligned lines, shown only on the stats page (cursor version read dynamically from the active snippet); nothing on the cursor badge. **Cursors `2026.09.12`** — real names when available: logged-in WP account, else the comment-author cookie name; otherwise a unique server-assigned "Anonyme N" remembered in localStorage (random fake first names removed, no more name collisions).
- 2026-09-04: **Live Cursors `2026.09.3`** — new snippet: shared live cursors across the whole site (AJAX polling), click confetti visible to everyone, WP display names for logged-in users / random names otherwise, "N online · X views" badge with dropdown listing each visitor and their page. **Site Stats Page `2026.09.2`** — standalone page at `/statistiques/` keeping the site header, hand-drawn SVG charts (cumulative/daily), year navigation, CSV import (Tools → Import Stats), view tracking in a dedicated table. **Super Tracker `2026.09.1`** — fixed the double `<?php` that broke tracking + security audit: removed 3 suspicious trackers (datapulse, histogram-analytics, swilty).
